...
import { Download, Folder, Sliders, ShieldAlert, Sparkles, Music, Film, Check } from 'lucide-react';
import {
  QualityPreset,
  ContainerOption,
  AudioQualityOption,
...
export const DownloadOptions: React.FC<DownloadOptionsProps> = ({
  ...
  const maxRes = metadata.maxDownloadableResolution || (metadata.availableResolutions.length > 0 ? metadata.availableResolutions[0] : 1080);

  // Check if selected quality is available
  const getQualityAvailability = (): { available: boolean; message?: string } => {
    if (quality === 'best' || quality === 'audio_only') return { available: true };
  ...
  const qualityOptions: { id: QualityPreset; label: string; minHeight?: number }[] = [
    ...
    { id: 'audio_only', label: 'Audio Only (MP3 / M4A)' },
  ];
  ...
  return (
    <div className="glass-panel rounded-2xl p-5 border border-slate-800/80 shadow-2xl space-y-5">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        ...
        <span className="text-xs text-slate-400 font-medium">Max Quality: <strong className="text-emerald-400">{maxRes}p</strong></span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
            ...
            {isAudioOnly ? (
              <span className="text-[10px] text-rose-400 font-normal">Audio Extracted</span>
            ) : (
              <span className="text-[10px] text-slate-400 font-normal">Verified Streams</span>
            )}
          </label>
          <div className="relative">
            <select
              value={quality}
              onChange={(e) => {
                const newQ = e.target.value as QualityPreset;
                setQuality(newQ);
                if (newQ === 'audio_only' && container !== 'mp3' && container !== 'm4a') {
                  setContainer('mp3');
                } else if (newQ !== 'audio_only' && (container === 'mp3' || container === 'm4a')) {
                  setContainer('mp4');
                }
              }}
            ...
            >
              {qualityOptions.map((opt) => {
                ...
                if (opt.minHeight) {
                  if (isAvailable) {
                    suffix = optInfo?.videoCodec ? ` — ✓ Available (${optInfo.videoCodec})` : ' — ✓ Available';
                  } else {
                    suffix = ' — ✗ Unavailable';
                  }
                }
                ...
                return (
                  <option
                    key={opt.id}
                    value={opt.id}
                    className="bg-darkbg-900 text-slate-100 py-1"
                  >
                    {opt.label}{suffix}
                  </option>
                );
        ...
        <div className="space-y-1.5">
          ...
          <select
            value={container}
            onChange={(e) => {
              const newC = e.target.value as ContainerOption;
              setContainer(newC);
              if ((newC === 'mp3' || newC === 'm4a') && quality !== 'audio_only') {
                setQuality('audio_only');
              } else if (newC !== 'mp3' && newC !== 'm4a' && quality === 'audio_only') {
                setQuality('best');
              }
            }}
            className="w-full bg-darkbg-800 text-slate-100 text-xs font-medium rounded-xl border border-slate-700/80 p-2.5 pr-8 focus:outline-none focus:border-rose-500 transition-colors"
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-300">Audio Quality</label>
          <select
            value={audioQuality}
      ...
      <div className="pt-2 flex justify-end">
        <button
          ...
          className={`flex items-center space-x-2.5 px-8 py-3 rounded-xl text-sm font-bold tracking-wide transition-all duration-200 shadow-xl ${
            isDownloading || !availability.available
